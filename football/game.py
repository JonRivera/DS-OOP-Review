...
class Game:
    ...
    -----------------------------
    teams : list
        list of pip length 2, strings of tea names
    location : str
        city
    score : dict
        key - team name
    ...
    def safety(self, points=2):
        """records pts for a team that does a safety"""
        if team not in self.teams:
            raise ValueError('team parameter must be in self.teams')
        else:
    ...
    def get_winning_team(self):
        ...
        if self.score[self.teams[1]] == self.score[self.teams[0]]:
            self.touchdown(choice(self.teams))
        # self.teams[0]
...
teams0 = ['blue', 'red']
score0 = {'blue': 0, 'red': 0}
game0 = Game(teams0, 'sd', {'blue': 1, 'red': 1}, 10)
